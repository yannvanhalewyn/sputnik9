export default [
  {
    id: 0,
    title: "Entry One",
    videos: [
      { title: 'Get By', url: 'https://player.vimeo.com/video/153681118' },
      { title: 'O E A', url: 'https://player.vimeo.com/video/153741821' }
    ],
    performers: [
      { name: 'Marlon Penn',
        instrument: 'vocals / keys',
        thumb_id: 'artist_thumbs/marlonthumb.jpg',
        about: 'Zanger, gitarist, Master of Ceremonies of “MC”, toetsenist, synthfreak, componist en producer Marlon Penninkhof is in 1985 geboren in het mooie Groningen. Naast de vele verschillende dingen die hij doet aan de maak­kant van de muziek is Marlon ook actief als DJ, poëet, danser en maker van (vaak slechte) grappen. Deze muzikale duizendpoot heeft al zoveel dingen gedaan dat ik ze eigenlijk niet durf op te noemen, omdat ik bang ben dat ik iets vergeet, maar hier toch een poging: Marlon Penn, Panenkov, Hotter than Denzel, Dwaallumen, Knalland, Florian Wolff, Marten de Paepe, Amber Arcades, Polynation en nog veel meer!'
      },
      {
        name: 'Leon Sibum',
        instrument: 'guitar',
        thumb_id: 'artist_thumbs/leonthumb.jpg',
        about: 'Leon is zijn muzikale carriere begonnen als drummer en heeft aan de Herman Brood Academie drums gestudeerd waar hij ook is begonnen met gitaar spelen. Na de Herman Brood Academie heeft Leon gitaar gestudeerd op de Rockacademie waar hij, naast het spelen in de band Dahl, de fijne kneepjes van het vak geleerd heeft en waar hij in aanraking is gekomen met het spelen van veel verschillende genres en muziekstijlen. Nu is Leon vooral actief in het theater, speelt hij in de band Grenadeers en wordt hij hier en daar gevraagd voor studiosessies.'
      },
      {
        name: 'Alex van Damme',
        instrument: 'bass',
        thumb_id: 'artist_thumbs/alexthumb.jpg',
        about: 'Naast bassist bij Shaking Godspeed en Crying Boys Cafe is Alex van Damme ook componist en producer. Alex is in 2013 afgestudeerd van het Conservatorium van Amsterdam, afdeling Pop, waar hij in zijn laatste jaar het stokje overnam van Paul Diersen als bassist, toetsenist en backup vocalist bij Shaking Godspeed. Alex heeft voordat hij bij zijn huidige bands ging spelen al veel ervaring opgedaan met bands als Lesoir en 21 Eys of Ruby.'
      },
      { name: 'Lars Spijkervet', instrument: 'drums', thumb_id: 'artist_thumbs/larsthumb.jpg' }
    ],
    blog: {
      text: `Tijdens de sessie van 5 en 6 januari hebben we, zoals je misschien al opgevallen was, twee erg verschillende songs geschreven. Dit is naar mijn idee meteen een tof voorbeeld van hoe onvoorspelbaar het resultaat van de sessies is. Mede daarom schep ik ook geen verwachtingen van de sessies, maar laat ik de boel de boel en zorg dat iedereen zich op z’n gemak voelt en zijn ding kan doen.\n Voor de eerste sessie heb ik Marlon, Leon en Alex gekozen omdat zij qua muziekstijl enigszins overlap hebben met elkaar, maar met de muziek waar ze voornamelijk actief in zijn juist erg ver uit elkaar liggen. Alle drie hebben ook ervaring in het schrijven van muziek, wat in een project als Sputnik 9 van groot belang is. De beide nummers die geschreven zijn in deze sessie zijn dan ook tot stand gekomen uit ideeën van alle muzikanten die mee hebben gedaan aan deze sessie.`
    },
    lyrics: `Where were your manners\n when you said I should be dead and gone?\n Would it be strange if I refused to pick up the phone?\n Leave me alone\n Leave me\n\n We had a thing\n had a fling\n now it's over and\n no need to reminisce\n why don't you find another man?\n Go find another man\n Go find another\n\n Your sorry is useless\n Look at all my bruises\n Stop it now / stop it now / stop it now\n Walk along\n\n I can't believe you tried to go out and sabotage\n The healing process of my sad, sad, broken heart\n You tore it apart\n You tore me up\n\n\nWe had a blast\n while it lasted\n but it didn't, so\n I'm done dancing\n you're no longer in control\n You're not untouchable\n You're not\n\n Damn your amends\n this is where it ends\n This is it / this is it / this was it\n This was the lie\n\n You hit me in the face\n You don't know what grace is\n I'll get by / you'll get by / we'll get by\n Bye, bye, bye\n Bye bye`,
    photos: {
      shown: [
        'entry_1/SPUTNIK1003_ik0ort',
        'entry_1/SPUTNIK1006_dcpkvh',
        'entry_1/SPUTNIK1013_fwa6mz',
        'entry_1/SPUTNIK1017_qb2x1s'
      ],
      hidden: [
        'entry_1_hidden/SPUTNIK1002_oioxlz',
        'entry_1_hidden/SPUTNIK1004_mz57zx',
        'entry_1_hidden/SPUTNIK1005_msloc2',
        'entry_1_hidden/SPUTNIK1007_hvonf4',
        'entry_1_hidden/SPUTNIK1008_az2igj',
        'entry_1_hidden/SPUTNIK1009_v2jg5k',
        'entry_1_hidden/SPUTNIK1010_pavb55',
        'entry_1_hidden/SPUTNIK1011_jhl5no',
        'entry_1_hidden/SPUTNIK1012_xvwyod',
        'entry_1_hidden/SPUTNIK1014_rntgwi',
        'entry_1_hidden/SPUTNIK1015_z9z3wk',
        'entry_1_hidden/SPUTNIK1016_qy8lnf',
        'entry_1_hidden/SPUTNIK1018_zss6jr',
        'entry_1_hidden/SPUTNIK1019_r9oog9',
        'entry_1_hidden/SPUTNIK1020_wqzajv'
      ]
    },
    contributors: [
      {
        title: 'Engineers',
        body: `Michiel Claessen (opname)\nSipke Woudstra (opname)\nLars Spijkervet (mix)\nDaan Kandelaars (master)\n`
      },
      {
        title: 'Video',
        body: `Wouter van Uden: ­ Get By\nLars Spijkervet: ­ O E A`
      },
      {
        title: 'Studio',
        body: `Galloway Recording Studio\nNijmegen`
      }
    ]
  },
  {
    id: 1,
    title: "Entry Two",
    videos: [
      { title: 'Beautiful Lie', url: 'https://player.vimeo.com/video/156077047' },
      { title: "It Wasn't My Plan", url: 'https://player.vimeo.com/video/156077328' }
    ],
    performers: [
      { name: 'Seb Dokman',
        instrument: 'vocals / guitar',
        thumb_id: 'entry_2_hidden/SPUTNIK2023',
        about: 'De 23­-jarige Seb Dokman, zanger en gitarist van de door heel de wereld spelende grunge­rockband ‘Paceshifters’, uit Wijhe (Overijssel) schrijft niet alleen maar grunge en alternatieve muziek, hij heeft ook een “rustigere” kant die geïnspireerd is door artiesten als Ryan Adams, Steve Earl en Neil Young. Nadat Seb in 2012 afstudeerde van de Herman Brood Academie in Utrecht, heeft hij zijn volledige leven op zijn band gericht waardoor ze in vijf jaar drie albums uitbrachten en op festivals als Pinkpop, de Zwarte Cross en SXSW hebben gespeeld.' },
      {
        name: 'Igor Sekulovic',
        instrument: 'guitar',
        thumb_id: 'entry_2_hidden/SPUTNIK2017',
        about: 'Oprichter, gitarist, zanger en creatief leider van Projekt Rakija, Igor Sekulovic, is in 1995 op 10­-jarige leeftijd gevlucht uit zijn geboorteland Bosnië en heeft de rest van zijn jeugd in Nederland doorgebracht. Op 14­jarige leeftijd begon Igor met zijn muzikale carriere als gitarist en speelde voornamelijk rock muziek. Op latere leeftijd kwam Igor weer in contact met zijn roots toen hij balkan muziek (her)ontdekte, wat uiteindelijk heeft geleid tot zijn uit de hand gelopen ‘Projekt Rakija’ waarmee Igor al op verschillende festivals zoals Lowlands en Sziget heeft gespeeld en door heel Europa getourd heeft.'
      },
      {
        name: 'Peter Peskens',
        instrument: 'bass',
        thumb_id: 'entry_2_hidden/SPUTNIK2016',
        about: 'Peter Peskens komt uit Heythuysen (Limburg) is 9 jaar geleden begonnen met bassen en is zijn muzikale carriere begonnen in de punkmuziek. Peter heeft veel ervaring opgedaan op de Herman Brood academie, in bands als Mongue, en het conservatorium van Amsterdam, wat hem uiteindelijk op het podium bij Ellen ten Damme, Orgelvreten, Armand en Jan Akkerman op het podium heeft gebracht! Ondertussen is Peter met zowel The Mysterons als Jungle By Night bezig aan een nieuw album.'
      },
      { name: 'Lars Spijkervet', instrument: 'drums / backup vocals', thumb_id: 'artist_thumbs/larsthumb.jpg' }
    ],
    blog: {
      text: `Voor Entry 2 heb ik 3 jongens met zeer verschillende achtergronden bij elkaar gezet. Iedereen is dan wel begonnen in de rock/punk muziek, alleen Seb is daarin echt door gegaan en heeft met zijn band Paceshifters over de hele wereld gespeeld. Igor zit met zijn eigen ‘Projekt Rakija’ in de balkan georiënteerde muziek en speelt Peter met Jungle By Night een mix tussen afrobeat, ethio­jazz en funk.\nDe eerste dag hebben we een song gemaakt die voornamelijk rock als basis heeft, in de verses hebben we echter een wat dub/hip­hop sfeer neer kunnen zetten door de drums en gitaar. De tweede song is veel meer richting de elektronische muziek, wat voornamelijk komt doordat Igor op de tweede dag zijn synthesizer meegenomen had waardoor Peter zijn baslijn door een effect genaamd de ‘Micro synth’ heeft gespeeld.  Door Seb’s achtergrond in de grunge heeft het toch zo’n vibe gekregen, wat het nummer niet misstaat.`
    },
    lyrics: `- Beautiful Lie:\n\nI can see it in front of me\nSomehow can’t reach it, I’m nobody\nA cold breeze enters my room\nThese thoughts are messing up my mind\n\nDon’t know if its true..\n\nA beautiful lie\nThis delusion is mine\n\nSomething obvious about this, but still I can’t reach it\nIt’s making me loose my self­control, cut me loose\n\nDon’t know if its true..\n\nA beautiful lie\nThis delusion is mine\n\nYou’re not here, you’re not here\n\nA beautiful lie\nThis delusion is mine\n\n\n- It Wasn’t My Plan\n\nBeen listening to your stories\nbeen doing it for months\nIt was all the way for me\ntoo blind to see\n\nYou dragged me all along\ninto your science of right and wrong\nIt was all the way for me\ntoo blind to see\n\nYou got me higher, you got me higher\nyou got me good\n\nIt wasn’t my plan\n\nYou couldn’t get along\nyou keep telling me I\n\nIt wasn’t my plan`,
    photos: {
      shown: [
        'entry_2/SPUTNIK2021',
        'entry_2/SPUTNIK2022',
        'entry_2/SPUTNIK2027',
        'entry_2/SPUTNIK2031'
      ],
      hidden: [
        'entry_2_hidden/SPUTNIK2001',
        'entry_2_hidden/SPUTNIK2002',
        'entry_2_hidden/SPUTNIK2003',
        'entry_2_hidden/SPUTNIK2005',
        'entry_2_hidden/SPUTNIK2006',
        'entry_2_hidden/SPUTNIK2007',
        'entry_2_hidden/SPUTNIK2008',
        'entry_2_hidden/SPUTNIK2009',
        'entry_2_hidden/SPUTNIK2010',
        'entry_2_hidden/SPUTNIK2011',
        'entry_2_hidden/SPUTNIK2012',
        'entry_2_hidden/SPUTNIK2013',
        'entry_2_hidden/SPUTNIK2014',
        'entry_2_hidden/SPUTNIK2015',
        'entry_2_hidden/SPUTNIK2016',
        'entry_2_hidden/SPUTNIK2017',
        'entry_2_hidden/SPUTNIK2018',
        'entry_2_hidden/SPUTNIK2019',
        'entry_2_hidden/SPUTNIK2020',
        'entry_2_hidden/SPUTNIK2023',
        'entry_2_hidden/SPUTNIK2024',
        'entry_2_hidden/SPUTNIK2025',
        'entry_2_hidden/SPUTNIK2026',
        'entry_2_hidden/SPUTNIK2028',
        'entry_2_hidden/SPUTNIK2029',
        'entry_2_hidden/SPUTNIK2030',
      ]
    },
    contributors: [
      {
        title: 'Engineers',
        body: `Sebastiaan van Bijlevelt (opname)\nLars Spijkervet (opname en mix)\nDaan Kandelaars (master)`
      },
      {
        title: 'Video',
        body: 'Wouter van Uden:­ Beautiful Lie\nLars Spijkervet: It Wasn’t My Plan'
      },
      {
        title: 'Studio',
        body: `Galloway Recording Studio\nNijmegen`
      }
    ]
  }
]
